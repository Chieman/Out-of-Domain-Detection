...
# def get_batch_token(text, tokenizer):
...
#
#
# def prepare_transformer_input(batch, tokenizer):
#     batch_text = [b[0] for b in batch]
#     intents = [b[1] for b in batch]
#     is_ood = [b[2] for b in batch]
#
#     feat = get_batch_token(batch_text, tokenizer)
#     padded_seq = feat["input_ids"]
#     att_idxs = feat["attention_mask"]
#     return {
#             'seq': padded_seq,
...
#     return prepare_transformer_input(batch, bert_tok)
...
def bert_collate(bert_tok):
    def collate_fn(batch):
        ...
        # print(encoded)
        encoded = bert_tok.batch_encode_plus(
            idxs,
            max_length=150,
            return_tensors='pt',
            padding="max_length",
...
def run_experiment(config):
    ...
    trainer.fit(module)
    eval_trainer = Trainer(gpus=0, accelerator='cpu',
                           logger=logger,
                           )
