...
class BiLSTM(nn.Module):
    def __init__(self, embedding_matrix, BATCH_SIZE, HIDDEN_DIM, CON_DIM, NUM_LAYERS, n_class_seen, DO_NORM, ALPHA, BETA, OOD_LOSS, ADV, CONT_LOSS, norm_coef, cl_mode=1, lmcl=True, use_cuda=True, use_bert=False, sup_cont=False):
        ...
        if self.use_bert:
            print('Loading Bert...')
            self.bert_model = AutoModel.from_pretrained("vinai/phobert-base").to(self.use_cuda)
    ...
    def forward(self, seq, adv_features=None, label=None, sim=None, mode='ind_pre'):
        if mode == 'ind_pre' or mode == 'finetune':
            if self.use_bert:
                seq_embed = self.bert_model(**self.bert_tokenizer(seq, max_length=150, return_tensors='pt', padding="max_length", truncation=True).to(self.use_cuda))[0]
                seq_embed = self.dropout(seq_embed)
            ...
            if self.use_bert:
                seq_embed = self.bert_model(**self.bert_tokenizer(seq, max_length=150, return_tensors='pt', padding=True, truncation=True).to(self.use_cuda))[0]
                seq_embed = self.dropout(seq_embed)
                seq_embed = seq_embed.clone().detach().requires_grad_(True).float()
            else:
                seq_embed = self.embedding(seq)
